...
## Roadmap — Concepts (ยังไม่ implement)

ฟีเจอร์ด้านล่างออกแบบมาให้ทำงานได้ใน **frontend-only architecture** (ไม่มี backend, ใช้ localStorage + MQTT)

---

### Long-term Memory

**แนวคิด**: Agent จำข้อมูลที่เรียนรู้จากบทสนทนาข้ามเซสชั่น แล้ว inject เป็น context ให้ router ก่อนตัดสินใจเลือก tool

**สิ่งที่ควรจำ**:

| ประเภท | ตัวอย่าง |
|---|---|
| Device alias | "ไฟตัวใหญ่" → Arc Floor Lamp |
| User preference | "ชอบแสง 180 ตอนดูหนัง" |
| Correction history | "อย่าปิด AC ทุกตัว ปิดแค่ห้องนั่งเล่น" |
| Scene shortcut | "โหมดหนัง" → [dim 100, TV on, AC 25°] |

**โครงสร้าง**:
```js
// localStorage key: 'aiot_memory'
{
  aliases: { "ไฟตัวใหญ่": "living-room/liv-lamp/set" },
  preferences: ["prefers brightness 180 when watching"],
  corrections: ["turn off only living room AC, not all rooms"]
}
```

**วิธี inject**: เพิ่ม memory summary ใน router system prompt
```
[User Memory]
Aliases: "ไฟตัวใหญ่" = Arc Floor Lamp (living-room/liv-lamp/set)
Preferences: prefers brightness 180 for movie watching
Past corrections: turn off only living room AC, not all rooms
```

**Skills ใหม่ที่ต้องเพิ่ม**: `memory_save` — ให้ agent บันทึกข้อมูลลง localStorage เองได้เมื่อ user พูดสิ่งที่ควรจำ

---

### Time-based Trigger (One-shot Delay)

**แนวคิด**: User บอก agent ว่าให้ทำอะไรในอีก X นาที agent ตั้ง setTimeout แล้วทำเมื่อถึงเวลา

**ข้อจำกัด**: ทำงานได้เฉพาะตอนที่ browser tab ยังเปิดอยู่ — ยอมรับ tradeoff นี้เพราะ frontend-only

**ตัวอย่างการใช้**:
```
"ปิดไฟในอีก 30 นาที"      → schedule_delay 30m → mqtt_publish false
"เตือนฉันดูน้ำในอีก 1 ชั่วโมง" → schedule_delay 1h → notify_user
```
...
```json
{
  ...
  "parameters": {
    ...
    "action": { "type": "string", "description": "Tool name to call when timer fires" },
    "action_args": { "type": "object" },
    "label": { "type": "string", "description": "Human-readable description shown in UI" }
  }
}
```

**UI**: แสดง pending timers เป็น chips ที่ cancel ได้ก่อนหมดเวลา

---

### Confirmation Flow

**แนวคิด**: Skills ที่มีผลกระทบสูง (reboot เครื่อง, ปิดระบบทั้งบ้าน) ต้องให้ user กด confirm ก่อน execute — ไม่ผ่าน agent อีกรอบ

**วิธีทำ**: เพิ่ม `requireConfirm: boolean` ใน skill definition ใน `data.js`

```js
// ตัวอย่างใน DEFAULT_SETTINGS.skills
{ id: 'os_command', name: 'os_command', requireConfirm: true, ... }
```

**Flow**:
```
Agent calls os_command
      │
skill has requireConfirm: true?
      │ Yes
      ▼
UI แสดง confirmation dialog
"Agent ต้องการรัน: shutdown /s /t 0 บนเครื่อง Windows"
[Cancel]  [Confirm]
      │
   Confirm
      ▼
execute จริง → return result → responder
```

**ไม่กระทบ agent logic** — แก้เฉพาะ `createExecuteTool` กับ UI component
